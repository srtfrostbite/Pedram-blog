...
def register():
    ...
    if User.query.filter_by(email=email).first():
        flash("Your are already registered, please login instead.")
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        user_in_database = User.query.filter_by(email=user_email).first()

        if user_in_database:
            if check_password_hash(pwhash=user_in_database.password, password=user_password):
